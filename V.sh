...
start_script() {
  ...
  echo -e "${BOLD}${BLUE}[+] =============================================== [+]${NC}"
  echo -e ""
  echo -e "Script ini dibuat khusus untuk menginstall tema Arix dari GitHub."
  echo -e "Pastikan Pterodactyl sudah terinstall dengan benar."
  echo -e ""
  sleep 2
}
...
install_arix() {
  echo " "
  echo -n -e "${BOLD}Mulai instalasi tema Arix? (y/n): ${NC}"
  read confirmation
  if [[ "$confirmation" != [yY] ]]; then 
      echo -e "${BOLD}Instalasi dibatalkan.${NC}"; exit 0; 
  fi
  ...
  if [ ! -d "/var/www/pterodactyl" ]; then
    print_error "Direktori Pterodactyl tidak ditemukan. Install Pterodactyl terlebih dahulu!"
    exit 1
  fi

  print_info "[1/6] Menyiapkan Sistem & Mendownload File Arix..."
  ...
  if [ ! -f "arix.zip" ]; then
      print_error "Gagal mendownload arix.zip dari GitHub. Cek kembali link atau koneksi."
      exit 1
  fi

  print_info "[2/6] Mengekstrak File Tema..."
  unzip -o -q arix.zip
  rm arix.zip

  print_info "[3/6] Memperbarui Dependensi Panel (Composer)..."
  ...
  print_info "[4/6] Menjalankan Setup Database & Arix..."
  php artisan migrate --force
  php artisan arix
  
  print_info "[5/6] Membersihkan Cache..."
  ...
  print_info "[6/6] Membangun Aset Tema (Yarn Build)..."
  
  # Cek Node.js versi 22
  CURRENT_NODE_VER=$(node -v 2>/dev/null | cut -d'.' -f1 | sed 's/v//')
  if [[ "$CURRENT_NODE_VER" != "22" ]]; then
      print_warning "Node.js v22 tidak ditemukan. Menginstall Node.js v22..."
  ...
  # Memperbaiki error OpenSSL yang sering muncul
  export NODE_OPTIONS=--openssl-legacy-provider
  
  yarn install
  yarn run build:production

  # Set Ownership Terakhir
  chown -R www-data:www-data /var/www/pterodactyl/*

  print_success "Tema Arix berhasil diinstall dengan sempurna!"
  echo " "
  echo -e "${BOLD}${GREEN}[+] =============================================== [+]${NC}"
  echo -e "${BOLD}${GREEN}[+]          INSTALASI BERHASIL SELESAI             [+]${NC}"
  echo -e "${BOLD}${GREEN}[+]           Silahkan Refresh Panel Anda           [+]${NC}"
  echo -e "${BOLD}${GREEN}[+] =============================================== [+]${NC}"
  echo " "
  sleep 2
}
